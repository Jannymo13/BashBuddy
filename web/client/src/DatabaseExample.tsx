      ...
      <div className="card-inner">
        <div className="card-face card-front">
          ...
          <div className="meta-row">
            ...
            <span className="small muted">{new Date(entry.timestamp).toLocaleDateString()}</span>
          </div>
        </div>
...
function DatabaseExample() {
  ...
  const fetchTableData = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/db/query/requests');
      if (!response.ok) {
  ...
  if (loading) return <div className="db-loading">Loading database entries...</div>;
  if (error) return <div className="db-error">Error: {error}</div>;
  if (data.length === 0) return <div className="db-empty">No database entries available</div>;
